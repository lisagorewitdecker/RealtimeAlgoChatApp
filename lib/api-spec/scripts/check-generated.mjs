...
import { buildDriftSummary, regenerationCommand } from "./drift-summary.mjs";
import {
...
let testSignalPending = false;
const testSignalDeliveryTimeoutMs = 5000;
...
function writeDriftReportFile(report) {
  // CI publishes this report as its own check run so reviewers can read the
  // evidence without job-log access. Writing the file is separate from the
  // step summary because each workflow step gets its own summary file.
  ...
  try {
    ...
    console.error(
      `Could not write the generated-drift report at ${reportPath}: ${describeError(error)}`,
    );
  }
}
...
// Signal delivery is asynchronous: the kernel may hand the signal to another
// thread and libuv reports it only once the event loop polls, so one timer
// tick is not always enough for a signal raised during the preceding
// synchronous work (seen intermittently with Node 24.20). A real interrupt
// that arrives later is still honored by the handler; an injected test signal
// is known to be on its way, so wait until the handler has recorded it before
// the exit status is decided.
async function yieldToSignals(cancellation) {
  await waitForEventLoopTurn();
  if (!testSignalPending) {
    return;
  }
  const deadline = Date.now() + testSignalDeliveryTimeoutMs;
  while (
    cancellation.interruptedSignal === undefined &&
    Date.now() < deadline
  ) {
    await waitForEventLoopTurn();
  }
  testSignalPending = false;
}
...
async function main() {
  ...
  try {
    ...
    await yieldToSignals(cancellation);
  } catch (error) {
    console.error(
