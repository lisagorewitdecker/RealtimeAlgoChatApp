...
function triggerTestSignal(checkpoint) {
  if (
    !testSignalTriggered &&
    process.env.API_CODEGEN_CHECK_TEST_SIGNAL_AT === checkpoint
  ) {
...
function findStagedBackupDirectories() {
  ...
  return [
    ...new Set(
      result.stdout
        ...
        .filter((directory) => directory.startsWith(backupDirectoryPrefix)),
    ),
  ].sort();
}
...
async function main() {
  ...
  try {
    stagedBackupDirectories = findStagedBackupDirectories();
  } catch (error) {
    console.error(
