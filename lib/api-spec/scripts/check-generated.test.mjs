...
function runChecker(
  ...
  if (driftReportPath) {
    env.API_CODEGEN_DRIFT_REPORT_PATH = driftReportPath;
  } else {
...
function extractDriftReport(output) {
...
test("writes the drift report to the GitHub step summary as a diff block", () => {
  withFixture("drift", ({ root, pnpmPath }) => {
    ...
    assert.notEqual(result.status, 0);
    const report = extractDriftReport(result.output);
...
test("writes the drift report to the configured report file", () => {
  withFixture("drift", ({ root, pnpmPath }) => {
    const driftReportPath = join(root, "drift-report.txt");
    const result = runChecker(root, pnpmPath, { driftReportPath });

    assert.notEqual(result.status, 0);
    assert.equal(
      readFileSync(driftReportPath, "utf8"),
      `${extractDriftReport(result.output)}\n`,
      "the report file must contain the same drift details shown in the job output",
    );
    assertGeneratedFilesRestored(root);
    assertBackupDirectoryRemoved(root);
  });
