...
const workspacePolicyPath = fileURLToPath(
  new URL("./codegen-check-workspace.mjs", import.meta.url),
);
const productionWorkspaceRoot = fileURLToPath(
  new URL("../../..", import.meta.url),
);
const normalizedProductionWorkspaceRoot = resolve(productionWorkspaceRoot);
...
if (behavior !== "no-drift") {
  ...
  writeFileSync(
    changedFile,
    behavior === "backtick-drift"
      ? String.fromCharCode(96).repeat(3) + " nested fence\\n"
      : "codegen output\\n",
  );
...
if (behavior === "many-file-drift") {
  for (let index = 0; index < 1200; index += 1) {
    const generatedFile = join(
      root,
      "lib/api-client-react/src/generated/many/file-" +
        String(index).padStart(4, "0") +
        ".ts",
    );
    mkdirSync(dirname(generatedFile), { recursive: true });
    writeFileSync(generatedFile, "export const regenerated = " + index + ";\\n");
  }
}
...
function runChecker(
  ...
  pnpmPath,
  {
    ...
    failCleanup = false,
    inheritedSignalAt,
    inheritedFailCleanup = false,
    inheritedRenderingError,
    enableTestWorkspace = true,
    renderingError,
    inheritedTerminationGraceMs,
    terminationGraceMs,
  } = {},
) {
  const env = {
  ...
  if (reportPath) {
    env.API_CODEGEN_DRIFT_REPORT_PATH = reportPath;
  } else {
    delete env.API_CODEGEN_DRIFT_REPORT_PATH;
  }
  if (signalAt || inheritedSignalAt) {
    env.API_CODEGEN_CHECK_TEST_SIGNAL_AT = signalAt ?? inheritedSignalAt;
  } else {
    delete env.API_CODEGEN_CHECK_TEST_SIGNAL_AT;
  }
  if (failCleanup || inheritedFailCleanup) {
    env.API_CODEGEN_CHECK_TEST_FAIL_CLEANUP = "1";
  } else {
    delete env.API_CODEGEN_CHECK_TEST_FAIL_CLEANUP;
  }
  const configuredRenderingError = renderingError ?? inheritedRenderingError;
  if (configuredRenderingError === undefined) {
    delete env.API_CODEGEN_CHECK_TEST_RENDERING_ERROR;
  } else {
    env.API_CODEGEN_CHECK_TEST_RENDERING_ERROR = configuredRenderingError;
  }
  const configuredTerminationGraceMs =
    terminationGraceMs ?? inheritedTerminationGraceMs;
  if (configuredTerminationGraceMs === undefined) {
    delete env.API_CODEGEN_CHECK_TERMINATION_GRACE_MS;
  } else {
    env.API_CODEGEN_CHECK_TERMINATION_GRACE_MS = String(
      configuredTerminationGraceMs,
    );
  }
  const checkerArguments = [
    ...
    terminationGraceMs !== undefined
      ? ["--enable-test-fault-injection"]
      : []),
  ];
  try {
    const output = execFileSync(process.execPath, checkerArguments, {
      cwd: root,
      encoding: "utf8",
      env,
    });
...
test("rejects an inherited test workspace without explicit opt-in", () => {
  withFixture("no-drift", ({ root, pnpmPath }) => {
    const result = runChecker(root, pnpmPath, { enableTestWorkspace: false });
    ...
    assert.equal(
      ...
      false,
      "codegen must not run for an inherited test workspace",
    );
...
test("keeps the production workspace when an empty override is inherited", () => {
  const outsideWorkspace = mkdtempSync(
    join(tmpdir(), "api-codegen-workspace-policy-"),
  );
  try {
    const result = spawnSync(
      ...
      [
        ...
        "-e",
        `
          const { resolveCodegenCheckWorkspace } = await import(
            ${JSON.stringify(pathToFileURL(workspacePolicyPath).href)}
          );
          const policy = resolveCodegenCheckWorkspace();
          process.stdout.write(JSON.stringify({
            workspaceRoot: policy.workspaceRoot,
            hasError: policy.error !== undefined,
          }));
        `,
      ],
      {
    ...
    assert.deepEqual(JSON.parse(result.stdout), {
      workspaceRoot: normalizedProductionWorkspaceRoot,
      hasError: false,
    });
  } finally {
...
test("ignores inherited test-only fault controls without explicit opt-in", () => {
  withFixture("no-drift", ({ root, pnpmPath }) => {
    const result = runChecker(root, pnpmPath, {
      inheritedSignalAt: "backup",
      inheritedFailCleanup: true,
      inheritedRenderingError: "inherited rendering failure",
      inheritedTerminationGraceMs: 1,
    });
...
for (const invalidTerminationGraceMs of ["0", "-1", "not-a-number", "100ms"]) {
  test(`rejects invalid test termination grace period ${JSON.stringify(invalidTerminationGraceMs)}`, () => {
    withFixture("no-drift", ({ root, pnpmPath }) => {
      const result = runChecker(root, pnpmPath, {
        terminationGraceMs: invalidTerminationGraceMs,
      });
      ...
      assert.equal(
        existsSync(join(root, codegenMarkerFile)),
        false,
        "codegen must not run with an invalid termination grace period",
      );
      assertGeneratedFilesRestored(root);
      assertBackupDirectoryRemoved(root);
    });
...
test("keeps signal status and names the leftover backup when interrupted cleanup fails", () => {
  withFixture("no-drift", ({ root, pnpmPath }) => {
    const result = runChecker(root, pnpmPath, {
      signalAt: "cleanup",
      failCleanup: true,
    ...
    assert.ok(
      result.output.includes(
        `- Removing the temporary backup directory ${backupDirectory} failed: simulated temporary-directory removal failure`,
      ),
      `output must identify the failed cleanup directory:\n${result.output}`,
    );
    assert.ok(
      result.output.includes(`Delete ${backupDirectory} manually.`),
      `output must explain how to remove the leftover directory:\n${result.output}`,
    );
  });
});
...
function startChecker(root, pnpmPath, { terminationGraceMs } = {}) {
  ...
  return spawn(process.execPath, checkerArguments, {
    cwd: root,
    env: {
      ...process.env,
      API_CODEGEN_CHECK_WORKSPACE: root,
...
test("writes the drift report to the GitHub step summary as a diff block", () => {
  withFixture("drift", ({ root, pnpmPath }) => {
    ...
    assert.notEqual(result.status, 0);
    const report = result.output.trimEnd();
    ...
    assert.equal(
      ...
      "the step summary must contain the same report and regeneration command as the job output",
    );
    assertGeneratedFilesRestored(root);
    assertBackupDirectoryRemoved(root);
  });
...
test("keeps generated backticks inside the reviewer-visible summary fence", () => {
  withFixture("backtick-drift", ({ root, pnpmPath }) => {
    const summaryPath = join(root, "step-summary.md");
    const result = runChecker(root, pnpmPath, { summaryPath });
    ...
    assert.match(
      summary,
      /^````diff\n[\s\S]*\+``` nested fence\n[\s\S]*\n````\n$/m,
      "the summary fence must be longer than any backtick run in generated content",
    );
    assertGeneratedFilesRestored(root);
    assertBackupDirectoryRemoved(root);
  });
...
test("keeps drift guidance visible when the GitHub step summary cannot be written", () => {
  withFixture("drift", ({ root, pnpmPath }) => {
    const summaryPath = join(root, "step-summary-directory");
    ...
    assert.match(
      result.output,
      /Could not write the GitHub step summary at .*step-summary-directory: /,
    );
    assert.match(
      result.output,
      /Run `pnpm --filter @workspace\/api-spec run codegen` and commit the generated output\./,
    );
    assert.doesNotMatch(result.output, /\n\s+at (appendFileSync|appendDriftSummary)/);
    assertGeneratedFilesRestored(root);
    assertBackupDirectoryRemoved(root);
  });
});
...
test("writes the drift report for publication when CI asks for one", () => {
  withFixture("drift", ({ root, pnpmPath }) => {
    const reportPath = join(root, "drift-report.txt");
    const result = runChecker(root, pnpmPath, { reportPath });

    assert.notEqual(result.status, 0);
    assert.equal(
      readFileSync(reportPath, "utf8"),
      `${result.output.trimEnd()}\n`,
      "the published report must match the report shown in the job output",
    );
    assertGeneratedFilesRestored(root);
    assertBackupDirectoryRemoved(root);
  });
...
test("does not write a drift report file during local drift checks", () => {
  withFixture("drift", ({ root, pnpmPath }) => {
    const result = runChecker(root, pnpmPath);

    assert.notEqual(result.status, 0);
    assert.equal(existsSync(join(root, "drift-report.txt")), false);
    assertGeneratedFilesRestored(root);
    assertBackupDirectoryRemoved(root);
  });
});

test("keeps drift guidance visible when the report file cannot be written", () => {
  withFixture("drift", ({ root, pnpmPath }) => {
    const reportPath = join(root, "drift-report-directory");
    ...
    assert.match(
      result.output,
      /Could not write the generated-drift report at .*drift-report-directory: /,
    );
    assert.doesNotMatch(result.output, /\n\s+at (writeFileSync|writeDriftReportFile)/);
    assertGeneratedFilesRestored(root);
    assertBackupDirectoryRemoved(root);
  });
});
...
test("bounds the complete failure when many generated files change", () => {
  withFixture("many-file-drift", ({ root, pnpmPath }) => {
    const result = runChecker(root, pnpmPath);
    ...
    assert.equal(
      outputLines.length,
      1000,
      `the complete generated-drift failure must use its configured 1000-line budget:\n${result.output}`,
    );
    assert.match(
      result.output,
      /\.\.\. 210 more changed file summary line\(s\) omitted \(showing first 992 of 1202 changed files\)\./,
    );
    assert.match(
      outputLines.at(-1),
      /Run `pnpm --filter @workspace\/api-spec run codegen` and commit the generated output\./,
...
test("keeps the drift-rendering fallback fixed-size and actionable", () => {
  withFixture("many-file-drift", ({ root, pnpmPath }) => {
    const result = runChecker(root, pnpmPath, {
      renderingError:
        "simulated rendering failure\nwith extra whitespace and path-like detail",
    });
    ...
    assert.equal(
      outputLines.length,
      6,
      `the rendering fallback must remain fixed-size:\n${result.output}`,
    );
    assert.match(result.output, /^1202 generated file\(s\) changed\.$/m);
    assert.match(
      result.output,
      /^\(Unable to render the bounded regeneration diff: simulated rendering failure with extra whitespace and path-like detail\)$/m,
    );
    assert.match(
      outputLines.at(-1),
      /Run `pnpm --filter @workspace\/api-spec run codegen` and commit the generated output\./,
...
test("keeps the drift-rendering fallback fixed-size in the GitHub step summary", () => {
  withFixture("many-file-drift", ({ root, pnpmPath }) => {
    const summaryPath = join(root, "step-summary.md");
    const result = runChecker(root, pnpmPath, {
      summaryPath,
    ...
    assert.equal(
      summaryLines.length,
      12,
      `the reviewer-visible rendering fallback must remain fixed-size:\n${summary}`,
    );
    assert.match(summary, /^1202 generated file\(s\) changed\.$/m);
    assert.match(
      summary,
      /^\(Unable to render the bounded regeneration diff: simulated rendering failure with extra whitespace and path-like detail\)$/m,
    );
