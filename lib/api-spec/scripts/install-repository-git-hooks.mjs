...
const dispatcherCandidateCleanupLimit = 32;
const dispatcherCandidateScanLimit = 128;
...
// expected_repository line.
function inspectRegistration(registrationPath) {
  let content;
  try {
    content = readFileSync(registrationPath, "utf8");
  } catch {
    return { state: "unreadable", target: null };
  }
  ...
  return {
    state: "valid",
    target: value.slice(1, -1).replaceAll("'\\''", "'"),
  };
}

// Anything unreadable or malformed never matches.
function registrationTarget(registrationPath) {
  return inspectRegistration(registrationPath).target;
}
...
// A registration whose recorded worktree is gone from disk (the developer
// removed the worktree without unregistering first) can never run again.
// Remove it so it cannot keep the dispatcher installed forever or block
// restoration of the original hook. A registration whose target cannot be
// read at all is left alone: an unreadable file does not prove the worktree
// is gone, and removing a live registration would silently unprotect it.
// A registration may only be pruned when its recorded worktree is proven
// gone. existsSync also reports false for access and other filesystem errors,
// so only ENOENT/ENOTDIR count as proof; anything else (EACCES, EIO, an
// unavailable mount) defers pruning instead of risking a live registration.
function targetIsDefinitelyMissing(target) {
  try {
    statSync(target);
    return false;
  } catch (error) {
...
function pruneStaleRegistrations(registrationsDirectory) {
  if (!existsSync(registrationsDirectory)) {
    return;
  }
  for (const name of readdirSync(registrationsDirectory)) {
    if (!name.endsWith(".sh")) {
      continue;
    }
    const registrationPath = join(registrationsDirectory, name);
    const target = registrationTarget(registrationPath);
    if (target === null || !targetIsDefinitelyMissing(target)) {
      continue;
    }
    rmSync(registrationPath, { force: true });
    // Drop the pruned registration from the shared record as well so it does
    // not outlive the file it points at. This is best-effort housekeeping:
    // the full unregister cleanup removes every value once no live
    // registration remains.
    try {
      const surviving = readGitConfigValues(dispatcherConfigKey).filter(
        (value) => value !== registrationPath,
      );
      writeGitConfigValues(dispatcherConfigKey, surviving);
    } catch {
      // A stale config value is harmless; the file removal above is what
      // keeps the dispatcher's registration set truthful.
    }
  }
}

// Read-only scan used by the plain setup path: only when a stale registration
// actually exists does setup try to take the dispatcher lock and prune, so a
// regular pnpm install with nothing to clean up never writes anything.
function hasStaleRegistrations(registrationsDirectory) {
  if (!existsSync(registrationsDirectory)) {
    return false;
  }
  let names;
  try {
    names = readdirSync(registrationsDirectory);
  } catch {
    return false;
  }
  return names.some((name) => {
    ...
    const target = registrationTarget(join(registrationsDirectory, name));
    return target !== null && targetIsDefinitelyMissing(target);
  });
}
...
function candidateOwnerIsRunning(candidatePath, candidatePid) {
  let owner;
  try {
    owner = JSON.parse(readFileSync(join(candidatePath, "owner"), "utf8"));
  } catch {
    // The candidate name records its creator before the owner file is written.
    // Keep an ownerless candidate while that creator is still running.
    return processIsRunning(candidatePid);
  }
  if (
    typeof owner?.pid !== "number" ||
    !Number.isInteger(owner.pid) ||
    owner.pid !== candidatePid ||
    owner.pid <= 0
  ) {
    return processIsRunning(candidatePid);
  }
...
function pruneAbandonedDispatcherLockCandidates(directory, lockPath) {
  const candidatePrefix = `${dispatcherLockName}-`;
  let inspected = 0;
  let scanned = 0;
  const entries = opendirSync(directory);
  try {
    while (scanned < dispatcherCandidateScanLimit) {
      const entry = entries.readSync();
      if (entry === null) {
        break;
      }
      scanned += 1;
      if (inspected >= dispatcherCandidateCleanupLimit) {
        break;
      }
      const name = entry.name;
      if (!entry.isDirectory() || !name.startsWith(candidatePrefix)) {
        continue;
      }
      const match = name
        .slice(candidatePrefix.length)
        .match(/^([1-9]\d*)-([0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})$/i);
      if (!match) {
        continue;
      }
      inspected += 1;
      ...
      if (candidateOwnerIsRunning(candidatePath, candidatePid)) {
        continue;
      }
      ...
      try {
        ...
        if (error?.code !== "ENOENT") {
          // Cleanup is best effort and must not prevent lock acquisition.
        }
      }
    }
  } finally {
...
function withDispatcherLock(configuredPath, action) {
  ...
  pruneAbandonedDispatcherLockCandidates(directory, lockPath);
  ...
  try {
    ...
    releaseDispatcherLock(lockPath, token);
  }
}

function releaseDispatcherLock(lockPath, token) {
  try {
...
// Non-blocking variant of withDispatcherLock for housekeeping from the plain
// setup path: returns false instead of waiting when another hook operation
// holds the lock, so a regular pnpm install is never slowed down by pruning.
function tryWithDispatcherLock(configuredPath, action) {
  const directory = hooksDirectory(configuredPath);
  const lockPath = join(directory, dispatcherLockName);
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      // One stale-lock reclamation attempt; a still-held lock skips pruning.
      if (!removeStaleDispatcherLock(lockPath)) {
        return false;
      }
    }
...
// Best-effort pruning from the plain setup path. It must never fail or
// noticeably slow down a regular pnpm install: the lock is only taken when
// the read-only scan found a stale registration, and a contended lock or any
// filesystem error simply defers pruning to a later run.
function tryPruneStaleRegistrations(configuredPath) {
  const registrationsDirectory = join(
    hooksDirectory(configuredPath),
    registrationsDirectoryName,
  );
  try {
    if (!hasStaleRegistrations(registrationsDirectory)) {
      return;
    }
    tryWithDispatcherLock(configuredPath, () =>
      pruneStaleRegistrations(registrationsDirectory),
    );
  } catch {
    // Pruning is housekeeping; the plain setup path must never fail on it.
  }
}
...
function installDispatcher(configuredPath) {
  ...
  try {
    ...
    for (const [path, label] of [
      [dispatcherTemporaryPath, "temporary dispatcher copy"],
      [registrationTemporaryPath, "temporary repository registration copy"],
    ]) {
      try {
        rmSync(path, { force: true });
      } catch (cleanupError) {
        cleanupFailures.push({ path, label, error: cleanupError });
      }
    }
    let registrationRollbackError = null;
    const registrationRestoreTemporaryPath = cleanupFailures.some(
      ({ path }) => path === registrationTemporaryPath,
    )
      ? `${registrationTemporaryPath}.rollback-${randomUUID()}`
      : registrationTemporaryPath;
    try {
      if (previousRegistration) {
        writeFileSync(
          registrationRestoreTemporaryPath,
          previousRegistration.content,
          {
            mode: previousRegistration.mode,
            flag: "wx",
          },
        );
        chmodSync(registrationRestoreTemporaryPath, previousRegistration.mode);
        renameSync(
          registrationRestoreTemporaryPath,
          registrationPath,
        );
    ...
    if (!alreadyManaged && installedDispatcher) {
      ...
      try {
        ...
        cleanupFailures.push({
          path: preservedPath,
          label: "preserved developer hook cleanup copy",
          error: cleanupError,
        });
      }
    }
    ...
    if (
      ...
      for (const failure of cleanupFailures) {
        recovery.push(
          `Unable to remove the ${failure.label}: ${failure.error instanceof Error ? failure.error.message : String(failure.error)}`,
          `The ${failure.label} may remain at "${failure.path}".`,
          `After resolving the filesystem or permission error, remove it manually with: rm -f ${shellQuote(failure.path)}`,
        );
      }
      if (registrationRollbackError) {
        recovery.push(
          ...
          previousRegistration
            ? `The previous registration recovery copy may remain at "${registrationRestoreTemporaryPath}". After resolving the filesystem or permission error, restore it manually with: mv -f ${shellQuote(registrationRestoreTemporaryPath)} ${shellQuote(registrationPath)}`
...
function collectDispatcherStatus(configuredPath) {
  const repositoryRoot = resolve(workspace);
  const directory =
    configuredPath === null
  ...
  const registrationInspection = registrationPresent
    ? inspectRegistration(registrationPath)
    : { state: "absent", target: null };
  const dispatcherRecords = readGitConfigValues(dispatcherConfigKey);
  const dispatcherHashRecord = readGitConfig(dispatcherHashConfigKey);
  const preservedPresent = existsSync(preservedPath);
  const registrationTargetsHere =
    registrationPresent &&
    registrationInspection.target === repositoryRoot;
  const registrationRecorded = dispatcherRecords.includes(registrationPath);
  const currentDispatcherHash =
    activeHook === "dispatcher"
      ? dispatcherHash(readFileSync(hookPath, "utf8"))
      : null;
  const warnings = [];

  if (registrationInspection.state === "unreadable") {
    warnings.push(
      `The repository registration file cannot be read. Inspect or replace the damaged record at "${registrationPath}"; status left it unchanged.`,
    );
  } else if (registrationInspection.state === "malformed") {
    warnings.push(
      `The repository registration file has malformed shell content. Inspect or replace the damaged record at "${registrationPath}"; status left it unchanged.`,
    );
  } else if (registrationPresent && !registrationTargetsHere) {
    warnings.push(
      `The repository registration file validly targets another worktree: "${registrationInspection.target}".`,
    );
  }
  if (registrationPresent && !registrationRecorded) {
    warnings.push(
      `The repository registration file is not listed in Git config ${dispatcherConfigKey}.`,
    );
  }
  if (!registrationPresent && registrationRecorded) {
    warnings.push(
      `Git config ${dispatcherConfigKey} lists this repository registration, but the file is missing.`,
    );
  }
  if (activeHook === "dispatcher" && !registrationPresent) {
    warnings.push(
      "The active dispatcher has no registration file for this repository, so its repository hook will not run.",
    );
  }
  for (const recordedPath of dispatcherRecords) {
    if (recordedPath !== "true" && !existsSync(recordedPath)) {
      warnings.push(
        `Git config ${dispatcherConfigKey} lists a missing registration file: "${recordedPath}".`,
      );
    }
  }
  if (activeHook === "dispatcher" && dispatcherHashRecord === null) {
    warnings.push("The dispatcher hash record is missing.");
  } else if (
    activeHook === "dispatcher" &&
    dispatcherHashRecord !== currentDispatcherHash
  ) {
    warnings.push(
      "The active dispatcher content does not match its recorded hash; preserve any edits before recovery.",
    );
  }
  if (
    activeHook !== "dispatcher" &&
    (registrationPresent ||
      dispatcherRecords.length > 0 ||
      dispatcherHashRecord !== null)
  ) {
    warnings.push(
      "Dispatcher cleanup is incomplete because installation records remain without an active dispatcher.",
    );
  }
  if (activeHook !== "dispatcher" && preservedPresent) {
    warnings.push(
      `The preserved developer hook remains at "${preservedPath}" without an active dispatcher.`,
    );
  }
  if (
    activeHook === "dispatcher" &&
    ...
    warnings.push(
      "The active dispatcher has no repository registration records.",
    );
  }

  // Every registration in the shared directory, not just this worktree's own,
  // so a registration left behind by a deleted worktree is visible before the
  // next install or uninstall prunes it. This listing is read-only; status
  // never prunes.
  const registrations = existsSync(registrationsDirectory)
    ? readdirSync(registrationsDirectory)
        .filter((name) => name.endsWith(".sh"))
        .sort()
        .map((name) => {
          const path = join(registrationsDirectory, name);
          const inspection = inspectRegistration(path);
          return {
            path,
            ...
            worktreePresent:
              inspection.target === null
                ? null
                : existsSync(inspection.target),
            thisRepository: inspection.target === repositoryRoot,
          };
        })
    : [];
  ...
  if (warnings.length > 0) {
    consistency =
      ...
      (registrationPresent ||
        ...
        : "inconsistent";
    guidance =
      consistency === "partially cleaned"
        ? 'After reviewing the records above, retry cleanup with "pnpm run hooks:dispatcher:uninstall".'
        : 'Review the warnings above before using "pnpm run hooks:dispatcher:install" or "pnpm run hooks:dispatcher:uninstall"; status made no changes.';
  } else if (activeHook === "dispatcher") {
    const shared =
      existsSync(registrationsDirectory) &&
      readdirSync(registrationsDirectory).filter((name) => name.endsWith(".sh"))
        .length > 1;
    consistency = shared ? "shared" : "installed";
    guidance = shared
      ? "This repository is consistently registered with a shared dispatcher."
      : "This repository dispatcher installation is consistent.";
  } else {
    consistency = "clean";
  ...
  return {
    ...
    preservedHook: {
      present: preservedPresent,
      path: preservedPath,
    },
    repositoryRegistration: {
      present: registrationPresent,
      path: registrationPath,
      state: registrationInspection.state,
      target: registrationInspection.target,
    },
    registrations,
    gitConfig: {
      [dispatcherConfigKey]: dispatcherRecords,
      [dispatcherHashConfigKey]: dispatcherHashRecord,
    },
    consistency,
    warnings,
    guidance,
...
function reportDispatcherStatus(configuredPath) {
  const status = collectDispatcherStatus(configuredPath);
  if (jsonOutput) {
  ...
  process.stdout.write(
    [
      ...
      `Active pre-commit hook: ${status.activeHook.state} ("${status.activeHook.path}")`,
      `Preserved developer hook: ${status.preservedHook.present ? `present ("${status.preservedHook.path}")` : "absent"}`,
      `Repository registration file: ${
        status.repositoryRegistration.present
          ? `present, ${status.repositoryRegistration.state} ("${status.repositoryRegistration.path}")`
          : `absent ("${status.repositoryRegistration.path}")`
      }`,
      ...(status.registrations.length === 0
        ? ["Shared registrations: none"]
        : [
            `Shared registrations (${status.registrations.length}):`,
            ...status.registrations.map((registration) => {
              if (registration.state !== "valid") {
                return `- "${registration.path}" (${registration.state}; status left it unchanged)`;
              }
              if (registration.worktreePresent === false) {
                return (
                  `- "${registration.path}" targets missing worktree "${registration.target}"; ` +
                  "the next dispatcher install or uninstall will remove this registration automatically"
                );
              }
              return `- "${registration.path}" targets live worktree "${registration.target}"${registration.thisRepository ? " (this repository)" : ""}`;
            }),
          ]),
      `Git config ${dispatcherConfigKey}: ${
        status.gitConfig[dispatcherConfigKey].length === 0
          ? "absent"
          : status.gitConfig[dispatcherConfigKey]
              .map((value) => JSON.stringify(value))
              .join(", ")
      }`,
      `Git config ${dispatcherHashConfigKey}: ${status.gitConfig[dispatcherHashConfigKey] === null ? "absent" : JSON.stringify(status.gitConfig[dispatcherHashConfigKey])}`,
...
function uninstallDispatcher(configuredPath) {
  ...
  const repositoryRoot = resolve(workspace);
  migrateLegacyRegistration(registrationsDirectory);
  // Registrations left behind by deleted worktrees must not count when
  // deciding whether this unregister is the final one.
  pruneStaleRegistrations(registrationsDirectory);
...
// The plain setup path is the only hook check that runs regularly (on every
// pnpm install), so it also prunes registrations left behind by deleted
// worktrees instead of waiting for someone to run a dispatcher command.
tryPruneStaleRegistrations(existingHooksPath);
...
process.stderr.write(
  [
    ...
    `The repository hook runs "pnpm run validate:repository-integrity" and "pnpm run validate:staged-codegen-backups".`,
