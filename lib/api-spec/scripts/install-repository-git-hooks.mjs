    ...
    if (
      ...
      if (registrationRollbackError) {
        recovery.push(
          ...
          previousRegistration
            ? `The previous registration recovery copy may remain at "${registrationRestoreTemporaryPath}". After resolving the filesystem or permission error, restore it manually by moving "${registrationRestoreTemporaryPath}" to "${registrationPath}", replacing the failed registration.`
...
function collectDispatcherStatus(configuredPath) {
  ...
  if (registrationPresent && !registrationTargetsHere) {
    warnings.push(
      `The repository registration file does not target "${repositoryRoot}".`,
    );
  }
    ...
    [
      ...
      `Repository registration file: ${
        status.repositoryRegistration.present
          ? `present ("${status.repositoryRegistration.path}")`
