...
function runHookInstaller(root, gitEnv, mode) {
  const result = spawnSync(process.execPath, [hookInstallerPath, mode].filter(Boolean), {
...
test("globally configured custom hooks are preserved without a local override", () => {
  withGitFixture((root, gitEnv) => {
    ...
    assert.match(install.output, /globally configured hooks directory/);
    assert.equal(existsSync(join(root, ".developer-global-hooks")), false);
