...
function installRepositoryHookFixture(root, gitEnv) {
  ...
  writeFileSync(
    ...
    `${JSON.stringify({
      scripts: {
        "validate:repository-integrity": "node -e \"process.exit(0)\"",
  ...
  () => {
    withGitFixture((root, gitEnv) => {
      ...
      assertJsonStatusIsReadOnly(root, gitEnv, hooks, (status) => {
        ...
        assert.equal(status.repositoryRegistration.state, "valid");
        assert.equal(status.repositoryRegistration.target, root);
...
test("dispatcher status lists a deleted worktree's registration without pruning it", () => {
  const fixture = mkdtempSync(join(tmpdir(), "stale-registration-status-"));
  const gitEnv = {
    ...process.env,
    GIT_CONFIG_GLOBAL: join(fixture, "global.gitconfig"),
    GIT_CONFIG_NOSYSTEM: "1",
  ...
  try {
    ...
    const secondRegistration = readdirSync(registrationsDirectory)
      .map((name) => join(registrationsDirectory, name))
      .find((path) => readFileSync(path, "utf8").includes(second));
    assert.ok(secondRegistration, "expected a registration for the second worktree");

    rmSync(second, { recursive: true, force: true });

    assertStatusIsReadOnly(first, gitEnv, hooks, (output) => {
      assert.match(output, /Shared registrations \(2\):/);
      assert.match(
        output,
        new RegExp(
          `targets live worktree "${first.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}" \\(this repository\\)`,
        ),
      );
      assert.match(
        output,
        new RegExp(
          `targets missing worktree "${second.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}"`,
        ),
      );
      assert.match(
        output,
        /next dispatcher install or uninstall will remove this registration automatically/,
      );
      // A leftover registration is informational, not an inconsistency: the
      // shared installation itself remains consistent.
      assert.match(output, /Consistency: shared/);
    });
    assertJsonStatusIsReadOnly(first, gitEnv, hooks, (status) => {
      assert.equal(status.registrations.length, 2);
      const live = status.registrations.find(
        (registration) => registration.thisRepository,
      );
      ...
      const stale = status.registrations.find(
        (registration) => !registration.thisRepository,
      );
      assert.deepEqual(stale, {
        ...
        thisRepository: false,
      });
    });
    // Status is read-only: the dead registration survives until the next
    // install or uninstall prunes it.
    assert.ok(existsSync(secondRegistration));
  } finally {
    rmSync(fixture, { recursive: true, force: true });
  }
});

test("dispatcher status diagnoses unreadable and malformed registration records without changes", () => {
  withGitFixture((root, gitEnv) => {
    execFileSync(
      "git",
      ["config", "--local", "core.hooksPath", ".developer-hooks"],
    ...
    assert.equal(runHookInstaller(root, gitEnv, "--install-dispatcher").status, 0);
    const registrationPath = execFileSync(
      "git",
      ["config", "--local", "--get", "replit.repositoryHooksDispatcher"],
      { cwd: root, env: gitEnv, encoding: "utf8" },
    ...
    mkdirSync(registrationPath);
    assertStatusIsReadOnly(root, gitEnv, hooks, (output) => {
      assert.match(output, /Repository registration file: present, unreadable/);
      assert.match(output, /registration file cannot be read/);
      assert.match(output, /Inspect or replace the damaged record/);
      assert.match(output, /status left it unchanged/);
      assert.doesNotMatch(output, /validly targets another worktree/);
    });
    assertJsonStatusIsReadOnly(root, gitEnv, hooks, (status) => {
      assert.equal(status.repositoryRegistration.state, "unreadable");
      assert.equal(status.repositoryRegistration.target, null);
    });

    rmSync(registrationPath, { recursive: true });
    writeFileSync(registrationPath, "#!/bin/sh\nexpected_repository=not-quoted\n");
    assertStatusIsReadOnly(root, gitEnv, hooks, (output) => {
      assert.match(output, /Repository registration file: present, malformed/);
      assert.match(output, /malformed shell content/);
      assert.match(output, /Inspect or replace the damaged record/);
      assert.match(output, /status left it unchanged/);
      assert.doesNotMatch(output, /validly targets another worktree/);
    });
    assertJsonStatusIsReadOnly(root, gitEnv, hooks, (status) => {
      assert.equal(status.repositoryRegistration.state, "malformed");
      assert.equal(status.repositoryRegistration.target, null);
    });
  });
});
...
test("dispatcher status warns about mismatched registration records, missing hashes, and edited content", () => {
  withGitFixture((root, gitEnv) => {
    ...
    writeFileSync(
      registrationPath,
      `#!/bin/sh\nexpected_repository='${otherWorktree}'\n`,
    );
    assertStatusIsReadOnly(root, gitEnv, hooks, (output) => {
      assert.match(output, /validly targets another worktree/);
      assert.match(
        output,
        new RegExp(otherWorktree.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")),
      );
      assert.doesNotMatch(output, /damaged record/);
    });
...
for (const failure of [
  {
    name: "dispatcher",
    match: 'String(target).includes(".pre-commit.replit-dispatcher-")',
    label: "temporary dispatcher copy",
  },
  {
    name: "registration",
    match: 'String(target).endsWith(".tmp")',
    label: "temporary repository registration copy",
  },
]) {
  test(
    `dispatcher install rollback restores the hook when ${failure.name} temporary cleanup fails`,
    () => {
      withGitFixture((root, gitEnv) => {
        execFileSync(
          "git",
        ...
        const original = "#!/bin/sh\nprintf original\n";
        writeFileSync(hook, original, { mode: 0o755 });
        const preload = join(root, `fail-${failure.name}-temporary-cleanup.mjs`);
        writeFileSync(
          preload,
          [
            'import childProcess from "node:child_process";',
            ...
            "fs.rmSync = (target, options) => {",
            `  if (${failure.match}) {`,
            '    fs.writeFileSync(target, "surviving temporary copy\\n");',
        ...
        assert.match(
          install.output,
          new RegExp(
            `Unable to remove the ${failure.label}: simulated temporary cleanup failure`,
          ),
        );
        assert.match(install.output, /original pre-commit hook was restored/);
        const survivingTemporaryPath = install.output.match(
          new RegExp(`The ${failure.label} may remain at "([^"]+)"`),
        )?.[1];
        assert.ok(survivingTemporaryPath);
        assert.equal(existsSync(survivingTemporaryPath), true);
        assert.equal(
          install.output.includes(
            `remove it manually with: rm -f '${survivingTemporaryPath}'`,
          ),
          true,
        );
        assert.equal(readFileSync(hook, "utf8"), original);
...
test("failed dispatcher reinstall preserves the working registration after temporary cleanup fails", () => {
  withGitFixture((root, gitEnv) => {
    execFileSync(
      "git",
      ["config", "--local", "core.hooksPath", ".developer-hooks"],
    ...
    writeFileSync(
      ...
      [
        ...
        '    fs.writeFileSync(target, "surviving registration temporary copy\\n");',
        '    throw Object.assign(new Error("simulated temporary cleanup failure"), { code: "EACCES" });',
        "  }",
        "  return rmSync(target, options);",
        "};",
    ...
    assert.match(
      reinstall.output,
      /Dispatcher installation failed: simulated config failure/,
    );
    assert.match(
      reinstall.output,
    ...
    assert.doesNotMatch(
      reinstall.output,
      /Unable to restore the previous repository registration/,
    );
    const survivingTemporaryPath = reinstall.output.match(
      /The temporary repository registration copy may remain at "([^"]+)"/,
    )?.[1];
    assert.ok(survivingTemporaryPath);
    assert.equal(existsSync(survivingTemporaryPath), true);
    assert.equal(
      reinstall.output.includes(
        `remove it manually with: rm -f '${survivingTemporaryPath}'`,
      ),
      true,
    );
...
test("failed dispatcher reinstall keeps a recoverable previous registration when restoration fails", () => {
  withGitFixture((root, gitEnv) => {
    execFileSync(
      "git",
      ["config", "--local", "core.hooksPath", ".developer-hooks"],
    ...
    assert.equal(firstInstall.status, 0, firstInstall.output);
    const hook = join(hooks, "pre-commit");
    const dispatcher = readFileSync(hook);
    const registrationPath = execFileSync(
      "git",
      ["config", "--local", "--get", "replit.repositoryHooksDispatcher"],
      { cwd: root, env: gitEnv, encoding: "utf8" },
    ).trim();
    const workingRegistration = `${readFileSync(registrationPath, "utf8")}# working registration marker\n`;
    writeFileSync(registrationPath, workingRegistration, { mode: 0o755 });
    const dispatcherHash = execFileSync(
      "git",
      ["config", "--local", "--get", "replit.repositoryHooksDispatcherHash"],
      { cwd: root, env: gitEnv, encoding: "utf8" },
    ...
    const preload = join(root, "fail-previous-registration-restore.mjs");
    writeFileSync(
      preload,
      [
        'import childProcess from "node:child_process";',
        ...
        "childProcess.spawnSync = (command, args, options) => {",
        '  if (command === "git" && args?.includes("replit.repositoryHooksDispatcherHash") && !args?.includes("--get")) {',
        '    return { status: 1, stdout: "", stderr: "simulated reinstall failure\\n" };',
        "  }",
        "  return spawnSync(command, args, options);",
        "};",
        "fs.renameSync = (source, destination) => {",
        '  if (String(source).endsWith(".tmp") && readFileSync(source, "utf8").includes("working registration marker")) {',
        '    throw Object.assign(new Error("simulated previous registration restore failure"), { code: "EACCES" });',
        "  }",
        "  return renameSync(source, destination);",
        "};",
        "syncBuiltinESMExports();",
    ...
    assert.match(
      reinstall.output,
      /Dispatcher installation failed: simulated reinstall failure/,
    );
    assert.match(
      reinstall.output,
      /Unable to restore the previous repository registration: simulated previous registration restore failure/,
    );
    assert.equal(
      reinstall.output.includes(
    ...
    assert.deepEqual(readFileSync(hook), dispatcher);
    assert.equal(readFileSync(registrationPath, "utf8").includes("working registration marker"), false);

    const recoveryPath = reinstall.output.match(
      /The previous registration recovery copy may remain at "([^"]+)"/,
    )?.[1];
    assert.ok(recoveryPath);
    assert.equal(existsSync(recoveryPath), true);
    assert.equal(readFileSync(recoveryPath, "utf8"), workingRegistration);
    assert.equal(
      reinstall.output.includes(
        `After resolving the filesystem or permission error, restore it manually with: mv -f '${recoveryPath}' '${registrationPath}'`,
      ),
      true,
    );
    execFileSync("mv", ["-f", recoveryPath, registrationPath]);
    assert.equal(readFileSync(registrationPath, "utf8"), workingRegistration);
...
test(
  "real macOS ps reclaims a reused PID lock and protects its active owner",
  { skip: process.env.REAL_MACOS_HOOK_LOCK_FIXTURE !== "1" },
  async () => {
    assert.equal(
      process.platform,
      "darwin",
      "REAL_MACOS_HOOK_LOCK_FIXTURE must run on macOS",
    );
    const startedAt = execFileSync(
      ...
      .trim()
      .replace(/\s+/g, " ");
    assert.notEqual(startedAt, "", "BSD ps must report this process start time");
    const startIdentity = `ps:${startedAt}`;
    ...
    for (const matchingOwner of [false, true]) {
      await withGitFixture(async (root, gitEnv) => {
        ...
        writeFileSync(
          ...
          `${JSON.stringify({
            ...
            startIdentity: matchingOwner
              ? startIdentity
              : `${startIdentity}-stale`,
            token: "real-macos-fixture-owner",
          })}\n`,
        );
        ...
        if (matchingOwner) {
          const finishedWhileOwnerWasActive = await Promise.race([
            install.then(() => true),
            new Promise((settle) => setTimeout(() => settle(false), 250)),
          ]);
          assert.equal(
            finishedWhileOwnerWasActive,
            false,
            "the matching live owner must retain its lock",
          );
...
test("plain setup prunes registrations left by deleted worktrees", () => {
  const fixture = mkdtempSync(join(tmpdir(), "plain-setup-prune-"));
  const gitEnv = {
    ...process.env,
    GIT_CONFIG_GLOBAL: join(fixture, "global.gitconfig"),
    GIT_CONFIG_NOSYSTEM: "1",
  ...
  const hooks = join(fixture, "shared-hooks");
  try {
    execFileSync("git", ["init", "--quiet", main], { cwd: fixture, env: gitEnv });
    execFileSync(
      "git",
      [
        "-c",
    ...
    for (const worktree of [main, sibling]) {
      mkdirSync(join(worktree, ".githooks"));
      writeFileSync(
        join(worktree, ".githooks", "pre-commit"),
        "#!/bin/sh\nexit 0\n",
    ...
    assert.equal(installSibling.status, 0, installSibling.output);
    const siblingRegistration = registrationPathOf(sibling);
    const mainRegistration = registrationPathOf(main);
    assert.equal(existsSync(siblingRegistration), true);

    // With nothing stale, a plain setup run stays read-only: no lock, no
    // writes, no record changes.
    const before = dispatcherStateSnapshot(main, gitEnv, hooks);
    const clean = runHookInstaller(main, gitEnv);
    assert.equal(clean.status, 0, clean.output);
    assert.deepEqual(dispatcherStateSnapshot(main, gitEnv, hooks), before);

    // The developer deletes the worktree folder without unregistering first.
    rmSync(sibling, { recursive: true, force: true });

    // While another hook operation holds the dispatcher lock, plain setup
    // still succeeds and leaves the stale registration for a later run.
    const lockPath = join(hooks, ".replit-repository-hooks.lock");
    mkdirSync(lockPath);
    writeFileSync(
      join(lockPath, "owner"),
      `${JSON.stringify({ pid: process.pid, token: "plain-setup-prune-fixture" })}\n`,
    );
    const contended = runHookInstaller(main, gitEnv);
    assert.equal(contended.status, 0, contended.output);
    assert.equal(existsSync(siblingRegistration), true);
    assert.equal(existsSync(lockPath), true);
    rmSync(lockPath, { recursive: true });

    // With the lock free, plain setup prunes the dead registration and drops
    // it from the shared dispatcher record.
    const pruned = runHookInstaller(main, gitEnv);
    assert.equal(pruned.status, 0, pruned.output);
    assert.equal(existsSync(siblingRegistration), false);
    assert.equal(existsSync(mainRegistration), true);
    const record = spawnSync(
      "git",
      ["config", "--local", "--get-all", "replit.repositoryHooksDispatcher"],
      { cwd: main, env: gitEnv, encoding: "utf8" },
    ...
    assert.deepEqual(record.stdout.trim().split("\n"), [mainRegistration]);
  } finally {
    rmSync(fixture, { recursive: true, force: true });
  }
});

test("plain setup keeps a registration whose worktree cannot be inspected", () => {
  const fixture = mkdtempSync(join(tmpdir(), "plain-setup-stat-fault-"));
  const gitEnv = {
    ...process.env,
    GIT_CONFIG_GLOBAL: join(fixture, "global.gitconfig"),
    GIT_CONFIG_NOSYSTEM: "1",
  ...
  try {
    ...
    // The sibling worktree still exists, but inspecting it fails with an I/O
    // error (as with an unavailable mount): plain setup must not treat that
    // as proof of deletion and must leave the registration untouched.
    const statFault = join(fixture, "fail-sibling-stat.mjs");
    writeFileSync(
      statFault,
      [
        'import fs from "node:fs";',
        ...
        `  if (String(path) === ${JSON.stringify(sibling)}) {`,
        "    const error = new Error('simulated I/O error');",
        "    error.code = 'EIO';",
        "    throw error;",
        "  }",
    ...
    const before = dispatcherStateSnapshot(main, gitEnv, hooks);
    const result = runHookInstaller(main, gitEnv, undefined, {
      NODE_OPTIONS: `--import=${statFault}`,
    });
    assert.equal(result.status, 0, result.output);
    assert.equal(existsSync(siblingRegistration), true);
    assert.deepEqual(dispatcherStateSnapshot(main, gitEnv, hooks), before);
    ...
    assert.deepEqual(
      ...
      [registrationPathOf(main), siblingRegistration].sort(),
    );
  } finally {
    rmSync(fixture, { recursive: true, force: true });
  }
