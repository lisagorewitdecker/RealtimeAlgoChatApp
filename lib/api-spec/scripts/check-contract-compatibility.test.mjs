...
test("summarizes blocked breaks with every changed contract and the override instructions", () => {
  ...
  assert.match(summary, /add both lines to the pull request description/);
  assert.match(
    summary,
...
test("keeps the full multi-line migration plan in the job summary", () => {
  const multiLineMigrationPlan = [
    "- Update the mobile client.",
    "- Keep v1 for one release.",
    "- Remove v1 afterwards.",
  ...
  assert.ok(summary.includes(markdownCodeForTest(multiLineMigrationPlan)), summary);
});

function markdownCodeForTest(value) {
  return `\`${value}\``;
}
