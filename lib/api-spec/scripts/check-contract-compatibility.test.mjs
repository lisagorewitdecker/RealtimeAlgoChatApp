...
const untrustedCheckerPath = fileURLToPath(
  new URL("../../../scripts/run-untrusted-checker.sh", import.meta.url),
);
...
function runProtectedCli(args, options = {}) {
  const { env = {}, summaryPath } = options;
  const childEnv = { ...process.env, ...env };
  for (const key of [
    "API_BREAKING_CHANGE_JUSTIFICATION",
    "API_BREAKING_CHANGE_MIGRATION_PLAN",
  ...
  return spawnSync(
    "bash",
    [untrustedCheckerPath, process.execPath, scriptPath, ...args],
    {
      encoding: "utf8",
      env: childEnv,
    },
...
test("guidance check bounds combined stale previews within one CI log budget", () => {
  const directory = mkdtempSync("/tmp/api-guidance-total-preview-");
  const paths = {
    workflow: join(directory, "api-codegen.yml"),
    contributorGuide: join(directory, "replit.md"),
    pullRequestTemplate: join(directory, "pull_request_template.md"),
  ...
  const oversizedLines = Array.from(
    { length: guidanceDiffLimits.maxLinesPerFile + 20 },
    (_, index) => `stale combined guidance line ${index}`,
  );
  writeFileSync(
    paths.workflow,
    readFileSync(workflowPath, "utf8").replace(
  ...
  try {
    ...
    assert.equal(
      lines.length,
      guidanceDiffLimits.maxTotalLines,
      "the complete failure must fit the configured guidance log budget",
    );
    assert.match(
      error.message,
      new RegExp(
        `\\.\\.\\. total guidance preview limit of ${guidanceDiffLimits.maxTotalLines} lines reached; \\d+ more preview line\\(s\\) across 2 stale guidance file\\(s\\) omitted\\.`,
      ),
    );
    assert.match(
      error.message,
...
test("guidance command rejects invalid repair arguments without modifying fixtures", () => {
  const directory = mkdtempSync("/tmp/api-guidance-cli-invalid-");
  const paths = {
    workflow: join(directory, "api-codegen.yml"),
    contributorGuide: join(directory, "replit.md"),
    pullRequestTemplate: join(directory, "pull_request_template.md"),
  ...
  const originals = {
    ...
    contributorGuide: readFileSync(contributorGuidePath, "utf8").replace(
      "Release the updated client first.",
      "Release a stale client first.",
    ),
    pullRequestTemplate: readFileSync(
      ...
      "utf8",
    ).replace("how existing consumers move", "how stale consumers move"),
  };
  for (const key of Object.keys(paths)) {
    writeFileSync(paths[key], originals[key]);
  }

  const validPathArguments = [
    "--workflow-file",
    paths.workflow,
    "--contributor-guide-file",
    paths.contributorGuide,
  ...
  const invalidInvocations = [
    {
      args: [...validPathArguments, "--unknown-guidance-option"],
      expectedError:
        /^Unknown argument: --unknown-guidance-option\nSupported options: --check, --workflow-file <path>, --contributor-guide-file <path>, --pull-request-template-file <path>\n$/,
    },
    ...[
      "--workflow-file",
      "--contributor-guide-file",
      ...
      args: [...validPathArguments, option],
      expectedError: new RegExp(`Missing value for ${option}`),
    })),
  ];

  try {
    for (const { args, expectedError } of invalidInvocations) {
      const result = spawnSync(process.execPath, [guidanceScriptPath, ...args], {
        encoding: "utf8",
      });
      ...
      for (const key of Object.keys(paths)) {
        assert.equal(
          ...
          originals[key],
          `${key} fixture must not change after invalid arguments`,
        );
      }
    }
  } finally {
...
test("guidance command repairs stale checked-in copies", () => {
  const directory = mkdtempSync("/tmp/api-guidance-cli-repair-");
  const paths = {
    workflow: join(directory, "api-codegen.yml"),
    contributorGuide: join(directory, "replit.md"),
    pullRequestTemplate: join(directory, "pull_request_template.md"),
  ...
  try {
    ...
    assert.match(
      result.stdout,
      /Updated generated API-break guidance in: workflow, contributorGuide, pullRequestTemplate\.\n$/,
    );
    assert.equal(
      readFileSync(paths.workflow, "utf8"),
      readFileSync(workflowPath, "utf8"),
...
test("warns about conflicting declarations without printing their values", () => {
  withBreakingSpecFixture(({ baselineFile, currentFile }) => {
    ...
    try {
      ...
      assertConflictWarningPrivacy({
        consoleOutput: result.stdout,
        markdownOutput: summary,
        expectedSources: [
          "command-line flag --breaking-change-justification",
          "environment variable API_BREAKING_CHANGE_JUSTIFICATION",
          "pull request body",
        ],
        declarationValues: [
          secretPullRequestValue,
          secretEnvironmentValue,
          flagValue,
          migrationPlan,
        ],
        description: "approved conflict warning",
      });
    } finally {
      rmSync(directory, { recursive: true, force: true });
    }
...
function assertConflictWarningPrivacy({
  consoleOutput,
  markdownOutput,
  expectedSources,
  declarationValues,
  ...
  const boundaries = [
    {
      ...
      warningPattern:
        /Warning: conflicting API-break declarations were supplied:[\s\S]*?declaration values are omitted from this warning\./i,
    },
    {
      name: "GitHub summary",
      output: markdownOutput,
      warningPattern:
        /### Declaration conflicts[\s\S]*?declaration values are omitted from this warning\./i,
    },
  ];

  return boundaries.map(({ name, output, warningPattern }) => {
    const warning = output.match(warningPattern)?.[0];
    assert.ok(
      warning,
      `${description} should include an isolated conflict warning in ${name} output`,
    );
    for (const source of expectedSources) {
      assert.match(
        warning,
        new RegExp(escapeRegExp(source)),
        `${description} should identify ${source} in ${name} output`,
      );
    }
    for (const value of declarationValues) {
      assert.doesNotMatch(
        ...
        `${description} must omit declaration values from ${name} output`,
      );
    }
    return warning;
  });
  ...
  withBreakingSpecFixture(({ baselineFile, currentFile }) => {
    ...
    try {
      for (const scenario of scenarios) {
        ...
        const warnings = assertConflictWarningPrivacy({
          consoleOutput: result.stdout,
          markdownOutput: readFileSync(summaryPath, "utf8"),
          expectedSources: [scenario.winnerSource, "pull request body"],
          declarationValues: [
            placeholder,
            planPlaceholder,
            pullRequestValue,
          ...
          description: `${scenario.name} approved conflict warning`,
        });
...
test("keeps rejected cross-source conflict warnings free of declaration values", () => {
  withBreakingSpecFixture(({ baselineFile, currentFile }) => {
    const directory = mkdtempSync("/tmp/api-compatibility-rejected-conflicts-");
    const placeholder = "<why the existing contract must break>";
    const planPlaceholder = "TBD";
    const pullRequestValue =
      "The pull request documents the incompatible API replacement.";
    const environmentValue =
      "The environment authorizes a separately reviewed API break.";
    const flagValue = "The command line authorizes the final API break.";
    const pullRequestBody = [
      `API_BREAKING_CHANGE_JUSTIFICATION: ${placeholder}`,
      `API_BREAKING_CHANGE_JUSTIFICATION: ${pullRequestValue}`,
    ...
    try {
      for (const scenario of scenarios) {
        ...
        for (const output of [result.stdout, summary]) {
          assert.match(
            output,
            /override was rejected because migration plan \(API_BREAKING_CHANGE_MIGRATION_PLAN\) is a placeholder/i,
          );
        }
        const warnings = assertConflictWarningPrivacy({
          consoleOutput: result.stdout,
          ...
          description: `${scenario.name} rejected conflict warning`,
        });
        for (const warning of warnings) {
          assert.match(warning, new RegExp(scenario.winnerSource));
          assert.doesNotMatch(warning, /ambiguous/);
          if (scenario.additionalConflictSource) {
            assert.match(
              warning,
              new RegExp(scenario.additionalConflictSource),
            );
          }
        }
      }
...
test("keeps rejected multiline cross-source conflict warnings free of declaration values", () => {
  withBreakingSpecFixture(({ baselineFile, currentFile }) => {
    const directory = mkdtempSync(
      "/tmp/api-compatibility-rejected-multiline-conflicts-",
    );
    const planPlaceholder = "<how existing consumers move to the new contract>";
    const pullRequestValue = [
      "The pull request documents the incompatible API replacement.",
      "The private rationale remains inside this multiline declaration.",
    ].join("\n");
    const environmentValue =
      "The environment authorizes a separately reviewed API break.";
    const flagValue = "The command line authorizes the final API break.";
    ...
    const scenarios = [
      {
        ...
        winnerSource: "environment variable API_BREAKING_CHANGE_JUSTIFICATION",
        additionalConflictSource: null,
      },
      {
        name: "flag winner",
    ...
    try {
      for (const scenario of scenarios) {
        ...
        for (const output of [
          ...
          assert.ok(
            warning,
            `${scenario.name} should include a blocked multiline conflict warning`,
          );
          assert.match(warning, new RegExp(scenario.winnerSource));
          ...
          for (const value of [
            ...
            assert.equal(
              ...
              false,
              `${scenario.name} conflict warning must not include declaration values`,
            );
          }
        }
      }
  ...
  withBreakingSpecFixture(({ baselineFile, currentFile }) => {
    ...
    try {
      ...
      const warnings = assertConflictWarningPrivacy({
        consoleOutput: result.stdout,
        markdownOutput: readFileSync(summaryPath, "utf8"),
        expectedSources: ["pull request body"],
        declarationValues: [justification, conflictingValue, migrationPlan],
        description: "inline conflict warning",
      });
      for (const warning of warnings) {
        assert.match(
          warning,
...
test("appends the summary only when a step summary file is configured", () => {
  ...
  const summaryPath = join(directory, "summary.md");
  const envSummaryPath = join(directory, "env-summary.md");
  try {
    process.env.GITHUB_STEP_SUMMARY = envSummaryPath;
...
test("keeps workflow commands in API review declarations inert", () => {
  withBreakingSpecFixture(({ baselineFile, currentFile }) => {
    const directory = mkdtempSync("/tmp/api-compatibility-workflow-command-");
    const summaryPath = join(directory, "summary.md");
    const maliciousDeclaration = "::error::forged API annotation";
    const result = runProtectedCli(
      ["--baseline-file", baselineFile, "--current-file", currentFile],
      {
        summaryPath,
        env: {
          API_BREAKING_CHANGE_PR_BODY: [
            `API_BREAKING_CHANGE_JUSTIFICATION: ${maliciousDeclaration}`,
            `API_BREAKING_CHANGE_MIGRATION_PLAN: ${migrationPlan}`,
          ].join("\n"),
        },
      },
    ...
    try {
      assert.equal(result.status, 0, result.stderr);
      const stopCommand = result.stdout.match(
        /^::stop-commands::([0-9a-f-]+)\n/,
      );
      assert.ok(
        stopCommand,
        "protected API compatibility output must begin with a stop-command fence",
      );
      const stopToken = stopCommand[1];
      const resumeCommand = `\n::${stopToken}::`;
      const resumeIndex = result.stdout.lastIndexOf(resumeCommand);
      const declarationIndex = result.stdout.indexOf(maliciousDeclaration);
      assert.ok(
        declarationIndex > stopCommand[0].length,
        "the malicious declaration must remain visible after command parsing is disabled",
      );
      assert.ok(
        resumeIndex > declarationIndex,
        "the workflow command parser must resume only after checker output",
      );
      assert.equal(
        result.stdout.match(/::error::/g)?.length,
        1,
        "the forged annotation must remain a single inert text occurrence",
      );
      ...
      assert.match(
        ...
        "the existing compatibility summary contract must remain intact",
      );
      assert.ok(
        summary.includes(
          `- Justification (\`API_BREAKING_CHANGE_JUSTIFICATION\`): \`${maliciousDeclaration}\``,
        ),
        "the summary must preserve the declaration as fenced review evidence",
      );
    } finally {
      rmSync(directory, { recursive: true, force: true });
    }
