...
test("the minimum complete-report budget is honored exactly", () => {
  const path = "generated.ts";
  const report = formatDriftReport({
    before: new Map([[path, fileFromLines(["before"])]]),
    after: new Map([[path, fileFromLines(["after"])]]),
    paths: [path],
    limits: { maxTotalLines: 8 },
  });

  assert.equal(report.split("\n").length, 8);
  assert.match(report, /- generated\.ts \(modified: \+1 -1\)/);
  assert.match(report, /1 changed file\(s\) omitted/);
  assert.match(report.split("\n").at(-1), /Run `pnpm .* codegen`/);
  assert.throws(
    () =>
      formatDriftReport({
        before: new Map(),
        after: new Map(),
        paths: [],
        limits: { maxTotalLines: 7 },
      }),
    /maxTotalLines must be an integer of at least 8/,
  );
});
...
test("path-bearing summary and omission lines elide long paths in the middle", () => {
  const path = `lib/api-zod/src/generated/${"deeply-nested/".repeat(12)}distinct-generated-schema.ts`;
  const change = describeGeneratedChange(
    path,
    fileFromLines(numberedLines(20)),
    fileFromLines(numberedLines(20, "regenerated")),
  );
  const limits = { maxLineLength: 100, maxLinesPerFile: 3 };

  const summary = summarizeGeneratedChange(change, limits);
  assert.ok(summary.length <= 101);
  assert.match(summary, /^- lib\/api-zod\/src\/generated\//);
  assert.match(summary, /….*distinct-generated-schema\.ts \(modified: \+20 -20\)$/);

  const omission = formatFileDiff(change, limits).at(-1);
  assert.ok(omission.length <= 101);
  assert.match(omission, /^\.\.\. 38 more diff line\(s\) omitted for lib\//);
  assert.match(
    omission,
    /….*ated-schema\.ts \(showing 3 of 41; limit 3 per file\)$/,
  );
});

test("file diff headers elide long added, removed, and modified paths", () => {
  const path = `lib/api-zod/src/generated/${"deeply-nested/".repeat(12)}distinct-generated-schema.ts`;
  const limits = { maxLineLength: 100 };
  const cases = [
    {
      kind: "added",
      before: undefined,
      after: fileFromLines(["fresh"]),
      expected: [/^--- \/dev\/null$/, /^\+\+\+ b\/lib\/api-zod\/src\/generated\/.*….*distinct-generated-schema\.ts$/],
    },
    {
      kind: "removed",
      before: fileFromLines(["gone"]),
      after: undefined,
      expected: [/^--- a\/lib\/api-zod\/src\/generated\/.*….*distinct-generated-schema\.ts$/, /^\+\+\+ \/dev\/null$/],
    },
    {
      kind: "modified",
      before: fileFromLines(["before"]),
      after: fileFromLines(["after"]),
      expected: [
        /^--- a\/lib\/api-zod\/src\/generated\/.*….*distinct-generated-schema\.ts$/,
        /^\+\+\+ b\/lib\/api-zod\/src\/generated\/.*….*distinct-generated-schema\.ts$/,
      ],
    },
  ];

  for (const { kind, before, after, expected } of cases) {
    const headers = formatFileDiff(
      describeGeneratedChange(path, before, after),
      limits,
    ).slice(0, 2);
    assert.ok(
      headers.every((header) => header.length <= limits.maxLineLength + 1),
      `${kind} headers stay within the configured width`,
    );
    assert.match(headers[0], expected[0]);
    assert.match(headers[1], expected[1]);
  }
});
...
test("whitespace legends stay within per-file and total output budgets", () => {
  ...
  const report = formatDriftReport({
    ...
    after,
    paths: [path],
    limits: { maxLinesPerFile: 3, maxTotalLines: 12 },
  });
  ...
  assert.match(report, /complete generated-drift failure limit of 12 lines reached/);

  const boundaryReport = formatDriftReport({
    before,
    after,
    paths: [path],
    limits: { maxLinesPerFile: 3, maxTotalLines: 13 },
  });
  assert.match(
    boundaryReport,
    /Whitespace: ⇥ tab/,
...
test("whitespace legends count their separator within the default complete-report budget", () => {
  const paths = Array.from(
    { length: 5 },
    (_, index) => `lib/api-zod/src/generated/large-whitespace-${index}.ts`,
  );
  const before = new Map(
    paths.map((path) => [
      path,
      fileFromLines(numberedLines(300).map((line) => `${line}\t`)),
    ]),
  );
  const after = new Map(
    paths.map((path) => [
      path,
      fileFromLines(numberedLines(300, "regenerated")),
    ]),
  );

  const report = formatDriftReport({ before, after, paths });
  const lines = report.split("\n");

  assert.equal(lines.length, defaultDriftReportLimits.maxTotalLines);
  assert.match(report, /^Whitespace: ⇥ tab$/m);
  assert.match(
    report,
    /complete generated-drift failure limit of 1000 lines reached/,
  );
  assert.match(lines.at(-1), /Run `pnpm .* codegen`/);
});
...
test("drift reports list every changed file and honor the total line budget", () => {
  ...
  assert.equal(boundedLines.length, 70, "the complete report honors its budget");
  assert.equal(
    boundedLines.at(-3),
    "... complete generated-drift failure limit of 70 lines reached; 59 more diff line(s) across 2 changed file(s) omitted.",
  );
});
...
test("drift reports reserve a concrete change from later files", () => {
  ...
  assert.match(
    report,
    /complete generated-drift failure limit of 34 lines reached; 52 more diff line\(s\) across 4 changed file\(s\) omitted/,
  );
  assert.equal(
    report.split("\n").length,
    34,
    "the complete report remains within the configured total budget",
  );
});

test("very small reports deterministically summarize files that cannot fit", () => {
  const paths = ["first.ts", "second.ts", "third.ts", "fourth.ts"];
  const before = new Map(
    paths.map((path, index) => [
      path,
      fileFromLines([index === 0 ? "old\t" : `old ${index}`]),
    ]),
  );
  const after = new Map(
    paths.map((path, index) => [path, fileFromLines([`new ${index}`])]),
  );
  const options = {
    before,
    after,
    paths,
    limits: { maxLinesPerFile: 10, maxTotalLines: 22 },
  };

  const report = formatDriftReport(options);
  const repeatedReport = formatDriftReport(options);

  assert.equal(
    repeatedReport,
    report,
    "whitespace legend reservation keeps the constrained allocation stable",
  );
  assert.equal(
    report.split("\n").length,
    22,
    "the truncation marker remains inside the complete report budget",
  );
  assert.match(report, /^Whitespace: ⇥ tab$/m);
  assert.match(report, /^--- a\/first\.ts$/m);
  assert.match(report, /^-old⇥$/m);
  assert.match(report, /^--- a\/second\.ts$/m);
  assert.match(report, /^\+\+\+ b\/second\.ts$/m);
  assert.doesNotMatch(report, /^-old 1$/m);
  assert.doesNotMatch(report, /^--- a\/third\.ts$/m);
  assert.doesNotMatch(report, /^--- a\/fourth\.ts$/m);
  assert.match(
    report,
    /complete generated-drift failure limit of 22 lines reached; 13 more diff line\(s\) across 4 changed file\(s\) omitted/,
  );
});
