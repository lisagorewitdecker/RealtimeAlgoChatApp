...
const declarationInstructionModel = Object.freeze({
  introduction:
    "For an intentional versioned break, add both declarations to the pull request description.",
  inlineExampleLabel: "Inline example",
  declarations: breakingChangeDeclarations,
  multiStepExampleLabel: "Multi-step migration plan example",
  multiStepDeclaration: breakingChangeDeclarations.find(
    (entry) => entry.key === "migrationPlan",
  ),
  multiStepItems: Object.freeze([
...
const pullRequestBodySource = "pull request body";
...
function formatDeclarationEvidence(declaration) {
  return [
    `Selected sources: ${breakingChangeDeclarations
      .map(
        (entry) =>
          `${entry.label} = ${declaration?.[`${entry.key}Source`] ?? "not supplied"}`,
      )
      .join("; ")}`,
...
function formatDeclarationConflictWarnings(declaration, { markdown = false } = {}) {
  const conflicts = declaration?.declarationConflicts ?? [];
  if (conflicts.length === 0) {
    return [];
  }
  const code = markdown ? markdownCode : (value) => value;
  return [
    "",
    markdown
      ? "### Declaration conflicts"
      : "Warning: conflicting API-break declarations were supplied:",
    ...conflicts.map(
      (conflict) =>
        conflict.ambiguousSource
          ? `- ${conflict.label} (${code(conflict.marker)}): ${code(conflict.ambiguousSource)} is ambiguous because it contains conflicting declarations`
          : `- ${conflict.label}: selected ${code(conflict.winnerSource)}, but a different value was supplied by ${conflict.conflictingSources
              .map(code)
              .join(" and ")}`,
    ),
    "Review the conflicting sources; declaration values are omitted from this warning.",
  ];
}

export function formatDeclarationInstructions() {
  const instructions = declarationInstructionModel;
  return [
    instructions.introduction,
    `${instructions.inlineExampleLabel}:`,
    ...instructions.declarations.map(
      (entry) => `  ${entry.marker}: ${entry.placeholder}`,
    ),
    "",
    `${instructions.multiStepExampleLabel}:`,
    `  ${instructions.multiStepDeclaration.marker}:`,
    ...instructions.multiStepItems.map((item) => `    - ${item}`),
    `  (${instructions.multiStepBoundary})`,
  ];
}

export function formatMarkdownDeclarationInstructions() {
  const instructions = declarationInstructionModel;
  return [
    instructions.introduction,
    "",
    ...
    ...instructions.multiStepItems.map((item) => `  - ${item}`),
    "```",
    instructions.multiStepBoundary.replace(
      "API_BREAKING_CHANGE_*",
      "`API_BREAKING_CHANGE_*`",
    ),
  ];
}

export function formatPullRequestDeclarationFields() {
  const instructions = declarationInstructionModel;
  return [
    "## API compatibility",
    "",
    `<!-- ${instructions.introduction} Replace the placeholders below when this pull request intentionally breaks the versioned API contract. -->`,
    ...instructions.declarations.map(
      (entry) => `${entry.marker}: ${entry.placeholder}`,
    ),
  ];
  ...
  return [
    ...
    ...(isIncompleteOverride(missing)
      ? [
          ...
          ...formatDeclarationConflictWarnings(declaration),
          "",
        ]
      : []),
...
function extractDeclarationValues(text, marker) {
...
export function extractBreakingChangeDeclaration(text) {
  return Object.fromEntries(
    breakingChangeDeclarations.map((entry) => {
      const values = extractDeclarationValues(text, entry.marker);
      return [
        entry.key,
        values.find((value) => declarationIssue(entry, value) === null) ??
          values[0] ??
          null,
      ];
    }),
...
function formatReviewEvidenceSection(declaration) {
  return [
    ...
    `Selected sources: ${breakingChangeDeclarations
      .map(
        (entry) =>
          `${entry.label} = ${markdownCode(declaration?.[`${entry.key}Source`] ?? "not supplied")}`,
      )
      .join("; ")}`,
    ...breakingChangeDeclarations.flatMap((entry) => {
      const value = declaration?.[entry.key];
      if (!value) {
        return `- ${entry.label} (${markdownCode(entry.marker)}): **missing**`;
      }
      if (value.includes("\n")) {
        return [
          `- ${entry.label} (${markdownCode(entry.marker)}):`,
          markdownCodeBlock(value),
        ];
      }
      return `- ${entry.label} (${markdownCode(entry.marker)}): ${markdownCode(value)}`;
    }),
  ];
}
    ...
    lines.push(
      ...
      ...(isIncompleteOverride(missing)
        ? [
            ...
            ...formatDeclarationConflictWarnings(result, { markdown: true }),
            "",
            `The intentional breaking change override was rejected because ${
              missing.every((entry) => entry.issue === "missing")
                ? `it omits the ${describeMissingDeclarations(missing)}`
...
export function writeStepSummary(
  ...
  const resolvedSummaryPath =
    arguments.length < 2 ? process.env.GITHUB_STEP_SUMMARY : summaryPath;
  if (!resolvedSummaryPath) {
    return false;
  }
  appendFileSync(resolvedSummaryPath, `${markdown}\n`);
  return true;
}
...
function parseArgs(argv) {
  ...
  for (const entry of breakingChangeDeclarations) {
    const environmentValue = process.env[entry.marker]?.trim() || null;
    options[entry.option] = environmentValue;
    options[`${entry.option}Source`] = environmentValue
      ? `environment variable ${entry.marker}`
      : null;
    options[`${entry.option}Declarations`] = environmentValue
      ? [
          {
            value: environmentValue,
            source: `environment variable ${entry.marker}`,
          },
        ]
      : [];
  }
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (argument === "--current-file") {
      ...
      options[`${entry.option}Source`] = `command-line flag ${entry.flag}`;
      options[`${entry.option}Declarations`].push({
        value,
        source: `command-line flag ${entry.flag}`,
      });
    } else {
...
function resolveBreakingChangeDeclaration(options) {
  ...
  const declared = extractBreakingChangeDeclaration(pullRequestBody);
  const resolved = {};
  const declarationConflicts = [];
  for (const entry of breakingChangeDeclarations) {
    const pullRequestValues = extractDeclarationValues(
      pullRequestBody,
      entry.marker,
    );
    const substantivePullRequestValues = pullRequestValues.filter(
      (value) => declarationIssue(entry, value) === null,
    );
    if (
      substantivePullRequestValues.length > 1 &&
      substantivePullRequestValues.some(
        (value) => value !== substantivePullRequestValues[0],
      )
    ) {
      declarationConflicts.push({
        label: entry.label,
        marker: entry.marker,
        ambiguousSource: pullRequestBodySource,
      });
    }
    const candidates = [];
    const trackedDeclarations = options[`${entry.option}Declarations`];
    if (Array.isArray(trackedDeclarations)) {
      candidates.push(...trackedDeclarations);
    } else {
      const explicitValue = options[entry.option]?.trim();
      if (explicitValue) {
        candidates.push({
          value: explicitValue,
          source:
            options[`${entry.option}Source`] ??
            `command-line flag ${entry.flag}`,
        });
      }
    }
    const pullRequestValue =
      substantivePullRequestValues[0] ??
      (candidates.length === 0 ? declared[entry.key] : null);
    if (pullRequestValue) {
      candidates.unshift({
        value: pullRequestValue,
        source: pullRequestBodySource,
      });
    }
    const winner = candidates.at(-1) ?? { value: null, source: null };
    resolved[entry.key] = winner.value;
    resolved[`${entry.key}Source`] = winner.source;
    const conflictingSources = candidates
      .slice(0, -1)
      .filter((candidate) => candidate.value !== winner.value)
  ...
  resolved.declarationConflicts = declarationConflicts;
  return resolved;
}
