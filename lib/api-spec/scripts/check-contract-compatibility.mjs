...
function missingDeclarations(declaration) {
  return breakingChangeDeclarations.filter(
    (entry) => !declaration?.[entry.key],
  );
}
...
export function formatCompatibilityFailure(
  ...
  const isIncompleteOverride =
    missing.length > 0 && missing.length < breakingChangeDeclarations.length;
  return [
    ...
    ...(isIncompleteOverride
      ? [
          "",
          `The intentional breaking change override was rejected because it omits the ${missing
...
function extractDeclarationValue(text, marker) {
  ...
  // The value must sit on the marker's own line so that an empty declaration
  // can never swallow the next marker line as its value.
