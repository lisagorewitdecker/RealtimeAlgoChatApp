...
export function formatDriftReport({ before, after, paths, limits = {} }) {
  ...
  while (true) {
    ...
    allocations = allocateFileLines(
      changes,
      fileDetails,
      Math.max(0, resolvedLimits.maxTotalLines - legendLines.length),
    );
  ...
  if (omittedFiles > 0) {
    output.push(
      `... total diff limit of ${resolvedLimits.maxTotalLines} lines reached; ${omittedLines} more line(s) across ${omittedFiles} changed file(s) omitted (every changed file is listed above).`,
    );
  }
