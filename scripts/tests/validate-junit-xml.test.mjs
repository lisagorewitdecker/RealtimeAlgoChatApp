...
test("accepts terminal escape bytes captured in device logs", () => {
  // Captured logs carry control bytes. They say nothing about whether the
  // upload is complete, so they must not fail a release.
  const escape = String.fromCharCode(0x1b);
  assert.equal(
    inspectJUnitReportText(
      `<testsuite name="x"><system-out><![CDATA[${escape}[31mred${escape}[0m]]></system-out></testsuite>`,
    ),
    JUNIT_REPORT_VALID,
  );
  assert.equal(
    inspectJUnitReportText(
      `<testsuite name="x"><system-out>${escape}[31mred</system-out></testsuite>`,
    ),
    JUNIT_REPORT_VALID,
  );
});
...
const notWellFormedReports = {
  ...
  "a raw < inside an attribute value": '<testsuite name="a<b"/>\n',
  ...
  "nesting past the supported depth": `${"<a>".repeat(70)}${"</a>".repeat(70)}`,
};
...
test("accepts terminal escape bytes captured in device logs", () => {
  // Captured logs carry control bytes in the places a runner writes log text.
  // They say nothing about whether the upload is complete, so they must not
  // fail a release.
  const escape = String.fromCharCode(0x1b);
  assert.equal(
    inspectJUnitReportText(
...
const notWellFormedReports = {
  ...
  // Characters that are illegal in XML wherever they appear.
  ...
  // Log bytes are tolerated only where a runner writes log text.
  "an escape byte inside a comment": "<testsuite><!-- \u001b[31m --></testsuite>",
  "an escape byte inside a processing instruction":
    "<testsuite><?maestro \u001b?></testsuite>",
...
test("file verdicts fail closed without inspecting the file", () => {
  ...
  try {
    ...
    // The shared 256 KiB release evidence bound applies to JUnit reports too.
    const oversizedPath = path.join(directory, "oversized-results.xml");
    writeFileSync(oversizedPath, `<testsuite name="${"x".repeat(262145)}"/>\n`);
    assert.equal(inspectJUnitReportFile(oversizedPath), JUNIT_REPORT_TOO_LARGE);
...
const notWellFormedReports = {
  ...
  "a non-character in text": "<testsuite>\uffff</testsuite>",
  "an unpaired surrogate in text": "<testsuite>\ud800</testsuite>",
  "a terminal escape byte in element text": `<testsuite><system-out>${ESCAPE}[31mred</system-out></testsuite>`,
  "a terminal escape byte inside CDATA": `<testsuite><system-out><![CDATA[${ESCAPE}[31mred]]></system-out></testsuite>`,
  "a terminal escape byte in an attribute value": `<testsuite><failure message="${ESCAPE}[31mexpected"/></testsuite>`,
  "a terminal escape byte inside a comment": `<testsuite><!-- ${ESCAPE}[31m --></testsuite>`,
  "a terminal escape byte inside a processing instruction": `<testsuite><?maestro ${ESCAPE}?></testsuite>`,
  // Release evidence is decoded as UTF-8, so no other encoding may be declared.
  "a declared encoding other than UTF-8":
    '<?xml version="1.0" encoding="ISO-8859-1"?><testsuite/>',
  // A namespace-aware parser rejects an unbound prefix, and JUnit reports use
