...
// Control characters that are illegal in XML. They are tolerated inside
// element text, attribute values, and CDATA (see below) but never in markup.
const MARKUP_CONTROL_PATTERN = /[\u0001-\u0008\u000b\u000c\u000e-\u001f]/;
const DECIMAL_PATTERN = /^[0-9]+$/;
const HEXADECIMAL_PATTERN = /^[0-9A-Fa-f]+$/;
...
// `<?xml version="1.0" encoding="UTF-8"?>` and the narrower forms the native
// runners emit. Anything else using the reserved `xml` target is rejected.
// Release evidence is decoded as UTF-8, so a report may not declare another
// encoding: the declaration has to agree with how the file is read.
const XML_DECLARATION_PATTERN =
...
/**
 * Reports whether `text` holds a character that is illegal anywhere in an XML
 * document: NUL, an unpaired surrogate (which also means the source bytes were
 * not valid UTF-8), or a non-character.
 */
function containsForbiddenCharacter(text) {
  for (let index = 0; index < text.length; index += 1) {
    const code = text.charCodeAt(index);
    if (code === 0x0 || code === 0xfffe || code === 0xffff) {
      return true;
    }
 ...
 * document, and no `]]>` in ordinary text.
 *
 * One deliberate departure from XML 1.0: control bytes are tolerated inside
 * element text, attribute values, and CDATA, because captured device logs
 * carry terminal escape sequences and their presence says nothing about
 * whether the upload is complete. Rejecting them would block a release on
 * captured log formatting rather than on missing evidence. They remain
 * illegal in markup, comments, and processing instructions, and NUL, unpaired
 * surrogates, and non-characters are rejected everywhere.
 *
 * A document type declaration is rejected. The native runners never emit one,
 * so its presence means the file did not come from the expected producer, and
 * without one no entity beyond the predefined five can be declared.
...
export function inspectJUnitReportText(reportText) {
  const text =
    reportText.charCodeAt(0) === 0xfeff ? reportText.slice(1) : reportText;
  if (containsForbiddenCharacter(text)) {
    return JUNIT_REPORT_NOT_WELL_FORMED;
  }
  const openElements = [];
  let index = 0;
  ...
  while (index < text.length) {
    ...
    if (text.startsWith("<!--", index)) {
      ...
      if (
        ...
        MARKUP_CONTROL_PATTERN.test(commentBody)
      ) {
        return JUNIT_REPORT_NOT_WELL_FORMED;
      }
      index = commentEnd + 3;
    ...
    if (text.startsWith("<?", index)) {
      ...
      const instruction = text.slice(index + 2, instructionEnd);
      if (MARKUP_CONTROL_PATTERN.test(instruction)) {
        return JUNIT_REPORT_NOT_WELL_FORMED;
      }
      const targetEnd = (() => {
        if (!NAME_START_PATTERN.test(instruction[0] ?? "")) {
...
/**
 * Reports whether `text` holds a character XML forbids anywhere in a document:
 * NUL and the other C0 control bytes, an unpaired surrogate (which also means
 * the source bytes were not valid UTF-8), or a non-character.
 *
 ...
 * nested elements, a single root, quoted attribute values with no raw `<`,
 * references that resolve to a predefined entity or a legal character,
 * terminated comments with no internal `--`, terminated CDATA sections,
 * processing instructions that do not reuse the reserved `xml` target, an XML
 * declaration only at the start of the document, and no `]]>` in ordinary
 * text.
 *
 * A document type declaration is rejected. The native runners never emit one,
 * so its presence means the file did not come from the expected producer, and
