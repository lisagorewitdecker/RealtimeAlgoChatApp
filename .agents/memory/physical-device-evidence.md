...
**Why:** On 2026-09-14 the audit showed none of the preconditions existed — no `mobile-release` environment, zero secrets/variables/runners/runs — and no EAS project at all (`eas.json`, bundle identifier, Android package, project ID missing), so re-probing phones or tokens locally would have missed the real blockers. The connection cannot read or push workflow files (see github-connection-workflow-scope.md); review the local git copy and prove hosted behaviour from a minimal token-pushed branch.
