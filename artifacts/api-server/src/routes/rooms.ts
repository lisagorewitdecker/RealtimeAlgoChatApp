...
socket.on('disconnect',()=>{aiRoomReady=false;updateAiRetryButton();if(!aiActive)return;aiActive=null;aiRetryAfterDisconnect=true;setAiBusy(false);setAiRetryAvailable(true);setAiStatus('Connection lost — the reply was interrupted. Retry when the room reconnects.','error')});
