...
const OG_IMAGE_SOURCE = path.join(
  projectRoot,
  "assets",
  "images",
  "og-image.png",
...
function copyOgImage() {
  const destination = path.join(projectRoot, "static-build", "og-image.png");

  if (!fs.existsSync(OG_IMAGE_SOURCE)) {
    throw new Error(`OG image not found: ${OG_IMAGE_SOURCE}`);
  }

  fs.copyFileSync(OG_IMAGE_SOURCE, destination);
  console.log("Copied OG image");
}

function createBrandAssets() {
  const iconPath = path.join(projectRoot, "assets", "images", "icon.png");
  const staticBuildRoot = path.join(projectRoot, "static-build");
  ...
  for (const [filename, size] of variants) {
    execFileSync(
      "magick",
      [iconPath, "-resize", size, "-strip", path.join(staticBuildRoot, filename)],
      { stdio: "pipe" },
    );
  }

  console.log("Created branded favicon and Apple touch icon");
}
...
async function startMetro(expoPublicDomain, expoPublicReplId) {
  ...
  if (clerkPublishableKey) {
    console.log("Setting EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY from the managed publishable-key environment variable");
  }
  if (sentryDsn) {
    console.log("Setting EXPO_PUBLIC_SENTRY_DSN from the managed Sentry environment variable");
  }
  if (expoPublicReplId) {
