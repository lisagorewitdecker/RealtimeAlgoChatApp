...
const IOS_EVIDENCE_ROOT = path.join(
  PACKAGE_ROOT,
  "test-results",
...
function resolveHandoffDirectory({ handoffDir, timestamp }) {
  const directory = handoffDir
    ? path.resolve(PACKAGE_ROOT, handoffDir)
    : path.join(
        IOS_EVIDENCE_ROOT,
        timestamp || formatTimestampDirectory(),
      );

  if (
    path.dirname(directory) !== IOS_EVIDENCE_ROOT ||
    !TIMESTAMP_DIRECTORY_PATTERN.test(path.basename(directory))
  ) {
    throw new Error(
      "The iOS handoff directory must be a UTC timestamp directory directly under the repository iOS evidence directory.",
    );
  }
...
function parseRetainedEvidence(contents) {
  ...
  for (const [index, line] of lines.entries()) {
    ...
    if (
      normalized.request &&
      normalized.request.platform === "ios" &&
...
export async function saveIosPreviewEvidence({
  ...
  const { nativeLines, normalizedLines } = parseRetainedEvidence(contents);
  const handoffDirectory = resolveHandoffDirectory({ handoffDir, timestamp });
  ...
  const nativePath = path.join(
    logsDirectory,
    "native-ios-request-evidence.txt",
  );
...
function printUsage() {
  console.log(`Save redacted iOS Expo Go preview request evidence.

Usage:
  pnpm run save:ios-preview-evidence -- [options]

Options:
  --source <path>        Retained Metro file; defaults to EXPO_DEV_REQUEST_EVIDENCE_FILE
                         or .expo/dev-request-evidence.log.
  --timestamp <UTC>      Create an iOS handoff directory such as 20260916T120000Z.
  --handoff-dir <path>   Use an existing or new timestamp directory directly under
                         artifacts/chat-app/test-results/encrypted-room-recovery/ios/.
  --help                 Show this help.
`);
}
...
async function main() {
  ...
  const result = await saveIosPreviewEvidence(options);
  ...
  console.log(
    `Saved filtered iOS Expo Go evidence: ${relativePath(result.nativePath)}`,
  );
