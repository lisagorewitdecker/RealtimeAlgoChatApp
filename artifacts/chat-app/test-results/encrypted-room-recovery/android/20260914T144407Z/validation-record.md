...
| Expo Go launch on physical Android | **BLOCKED** | No physical phone, device farm, self-hosted Android runner, or Expo Go request was available. |
...
the unavailable physical-phone and native-request boundaries are explicit
four-boundary handoff rows. It does not accept the public `curl` response as
an Android preview pass. The companion preflight sidecar records the same
public and local probe statuses using the current redacted JSON contract.
Run `pnpm run validate:android-preview-evidence -- <path>/validation-record.md`
