...
a `testsuite` element, is rejected on structure alone: the diagnostic names the
file path and the structural reason, and never quotes what the file contains.
Open the uploaded report itself to see why the run stopped. Characters XML
forbids — a NUL byte, or a raw terminal escape carried over from a captured
device log — are rejected wherever they appear, because a conforming parser
refuses the whole document over them; a runner that captures such bytes has to
strip them before writing the report.
